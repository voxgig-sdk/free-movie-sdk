...
class FreeMovieRetryFeature extends FreeMovieBaseFeature
{
    ...
    public function init(FreeMovieContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (FreeMovieContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(FreeMovieContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(FreeMovieContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
