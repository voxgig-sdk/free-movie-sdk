...
class FtCtrl extends FreeMovieControl
{
...
class FtClient
{
    ...
    private FreeMovieUtility $utility;

    public function __construct(FreeMovieUtility $utility, array $options)
    {
    ...
    public function get_utility(): FreeMovieUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (FreeMovieContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public FreeMovieContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (FreeMovieContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = FreeMovieConfig::make_feature($name);
    ...
    private static function buildUrl(FreeMovieSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof FreeMovieError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new FreeMovieResponse($fetched);
        }
    ...
    private function fail(FreeMovieContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (FreeMovieContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
