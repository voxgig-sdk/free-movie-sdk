...
{
    private static function utility(): FreeMovieUtility
    {
        return new FreeMovieUtility();
    }
    ...
    private static function ctx(array $over = []): FreeMovieContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new FreeMovieContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new FreeMovieOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof FreeMovieError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new FreeMovieOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = FreeMovieMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new FreeMovieOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = FreeMovieMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new FreeMovieOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = FreeMovieMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new FreeMovieSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = FreeMovieMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new FreeMovieResponse([]),
            'result' => new FreeMovieResult([]),
        ]);
        [, $err] = FreeMovieMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new FreeMovieSpec([]),
            'response' => null,
            'result' => new FreeMovieResult([]),
        ]);
        [, $err] = FreeMovieMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new FreeMovieSpec(['step' => 's']),
            'response' => new FreeMovieResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new FreeMovieResult([]),
        ]);
        [, $err] = FreeMovieMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new FreeMovieSpec(['step' => 's']),
            'response' => new FreeMovieResponse(self::resp(200, ['v' => 1])),
            'result' => new FreeMovieResult([]),
        ]);
        [, $err] = FreeMovieMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new FreeMovieSpec(['step' => 's']),
            'response' => new FreeMovieResponse(self::resp(200, ['v' => 2])),
            'result' => new FreeMovieResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        FreeMovieMakeResponse::call($ctx);
    ...
    {
        $preset = new FreeMovieResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new FreeMovieSpec([]),
            'response' => new FreeMovieResponse([]),
            'result' => new FreeMovieResult([]),
        ...
        [$got, $err] = FreeMovieMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new FreeMovieResult([])]);
        [, $err] = FreeMovieMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new FreeMovieSpec([]), 'result' => null]);
        [, $err] = FreeMovieMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new FreeMovieSpec(['step' => 's']),
            'result' => new FreeMovieResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new FreeMovieOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = FreeMovieMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new FreeMovieSpec(['step' => 's']),
            'result' => new FreeMovieResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new FreeMovieOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = FreeMovieMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new FreeMovieResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new FreeMovieSpec([]),
            'result' => new FreeMovieResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = FreeMovieMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = FreeMovieMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new FreeMovieError('boom', 'boom');
        $utility->fetcher = function (FreeMovieContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new FreeMovieSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = FreeMovieMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (FreeMovieContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = FreeMovieMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(FreeMovieResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (FreeMovieContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new FreeMovieSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = FreeMovieMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new FreeMovieResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new FreeMovieSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = FreeMovieMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = FreeMovieMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = FreeMovieMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new FreeMovieResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, FreeMovieDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new FreeMovieResult(['ok' => false])]);
        $this->expectException(FreeMovieError::class);
        FreeMovieDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new FreeMovieResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', FreeMovieMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        FreeMovieMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new FreeMovieBaseFeature();
        $b = new FreeMovieBaseFeature();
        FreeMovieFeatureAdd::call($ctx, $a);
        FreeMovieFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): FreeMovieBaseFeature
    {
        $f = new FreeMovieBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        FreeMovieFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): FreeMovieContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new FreeMovieSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = FreeMoviePrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new FreeMovieResponse(['headers' => null]),
            'result' => new FreeMovieResult([]),
        ]);
        FreeMovieResultHeaders::call($ctx);
