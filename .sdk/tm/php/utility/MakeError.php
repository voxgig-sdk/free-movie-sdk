...
class FreeMovieMakeError
{
    public static function call(?FreeMovieContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new FreeMovieContext([], null);
        }
        $op = $ctx->op ?? new FreeMovieOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new FreeMovieResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof FreeMovieError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new FreeMovieError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof FreeMovieError) {
