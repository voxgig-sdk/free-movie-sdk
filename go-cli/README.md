...
boru-driven command-line client **and** interactive REPL for the FreeMovie
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/free-movie-cli)
make build
...
./free-movie-cli load 1 movie            # {id:1} shorthand
./free-movie-cli load '{id:1}' movie       # explicit match map

# 5. Override the API base URL for a single call
FREE_MOVIE_BASE=https://api.example.com ./free-movie-cli load 1 movie

# 6. No arguments -> interactive REPL
./free-movie-cli
free-movie> load 1 movie
free-movie> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/free-movie-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/free-movie-cli load 1 movie
   ```
...
```sh
./free-movie-cli load 1 movie          # scalar shorthand for {id:1}
./free-movie-cli load '{id:1}' movie     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `free-movie>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `movie`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/free-movie-cli`. |
...
(`./free-movie-cli <expr>`) and interactively (the REPL), and expressions compose the
