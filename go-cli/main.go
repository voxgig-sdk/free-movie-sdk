// Command FreeMovie-cli is a CLI + REPL for the FreeMovie SDK,
...
const prompt = "free-movie"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "movie search"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      FREE_MOVIE_APIKEY (api key), FREE_MOVIE_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: FREE_MOVIE_APIKEY carries the API key and
	// FREE_MOVIE_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("FREE_MOVIE_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("FREE_MOVIE_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewFreeMovieSDK(opts)
