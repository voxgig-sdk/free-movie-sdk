...
[MCP](https://modelcontextprotocol.io) server exposing the FreeMovie SDK as
two agent tools — `free-movie_list` and `free-movie_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/free-movie-mcp)
make build

# 2. Provide credentials via the environment
export FREE_MOVIE_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user free-movie \
  -- /absolute/path/to/free-movie-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./free-movie-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// free-movie_list: first page of records
{ "entity": "search" }
{ "entity": "search", "query": { } }

// free-movie_load: one record by id
{ "entity": "movie", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/free-movie-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user free-movie \
     -- "$PWD"/dist/*/free-movie-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `free-movie_list` and `free-movie_load` tools now appear
   in new sessions. Ask the agent to *"list search using free-movie"*
   and it calls `free-movie_list` with `{"entity":"search"}`.
...
```sh
export FREE_MOVIE_APIKEY=sk_live_xxx            # API key
export FREE_MOVIE_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `free-movie_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `free-movie_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `FREE_MOVIE_APIKEY` | API key sent with every request. |
| `FREE_MOVIE_BASE` | Optional override of the API base URL. |
...
```sh
./free-movie-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"free-movie_load","arguments":{"entity":"movie","query":{"id":1}}}}'
```
