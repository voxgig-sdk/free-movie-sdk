...
MCP server exposing the FreeMovie SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `free-movie_list` | `entity`, optional `query` map | First page of records as JSON |
| `free-movie_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `free-movie_list` and
`free-movie_load` tools become available in new sessions.
