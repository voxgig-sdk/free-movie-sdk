...
func TestMovieEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set FREE_MOVIE_TEST_MOVIE_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		movieRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.movie")))
...
func movieBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("FREE_MOVIE_TEST_MOVIE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["FREE_MOVIE_TEST_MOVIE_ENTID"])
