...
import (
	...
	omni "github.com/voxgig-sdk/free-movie-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.FreeMovieError); is {
				out := map[string]any{
					"name":    "FreeMovieError",
...
func omniCtx(arg any, client *sdk.FreeMovieSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.FreeMovieSDK); is {
			client = live
