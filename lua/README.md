...
The Lua SDK for the FreeMovie API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Movie(nil):load({ id = "example_id" }, nil)
...
local result, err = client:FreeMovie(nil):load(
  { id = "test01" }, nil
)
