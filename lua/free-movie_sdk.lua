...
local FreeMovieSDK = {}
FreeMovieSDK.__index = FreeMovieSDK
...
FreeMovieSDK._make_feature = _make_feature


function FreeMovieSDK.new(options)
  local self = setmetatable({}, FreeMovieSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function FreeMovieSDK:options_map()
...
function FreeMovieSDK:get_utility()
  return Utility.copy(self._utility)
end


function FreeMovieSDK:get_root_ctx()
  return self._rootctx
end


function FreeMovieSDK:prepare(fetchargs)
...
function FreeMovieSDK:direct(fetchargs)
...
-- by passing it.
function FreeMovieSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function FreeMovieSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "FreeMovieSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function FreeMovieSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = FreeMovieSDK.new(sdkopts)
  sdk.mode = "test"
