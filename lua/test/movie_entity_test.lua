...
describe("MovieEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set FREE_MOVIE_TEST_MOVIE_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- Bootstrap entity data from existing test data.
    local movie_ref01_data_raw = vs.items(helpers.to_map(
      vs.getpath(setup.data, "existing.movie")))
    local movie_ref01_data = nil
    if #movie_ref01_data_raw > 0 then
      movie_ref01_data = helpers.to_map(movie_ref01_data_raw[1][2])
    end
...
function movie_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("FREE_MOVIE_TEST_MOVIE_ENTID")
  ...
  if env["FREE_MOVIE_TEST_LIVE"] == "TRUE" then
  ...
  local live = env["FREE_MOVIE_TEST_LIVE"] == "TRUE"
  return {
    client = client,
    data = entity_data,
