...
class SearchEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $base = FreeMovieSDK::test($seed, null);
        $seen = iterator_to_array($base->Search(null)->stream("list", null, null), false);
        $this->assertCount(3, $seen);

        // Inbound: streaming active -> yields each item from the feature.
        $cfg = FreeMovieConfig::shared_config();
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            $sdk = FreeMovieSDK::test($seed, ["feature" => ["streaming" => ["active" => true]]]);
            $got = [];
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set FREE_MOVIE_TEST_SEARCH_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function search_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("FREE_MOVIE_TEST_SEARCH_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["FREE_MOVIE_TEST_SEARCH_ENTID"]);
