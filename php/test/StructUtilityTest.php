...
// FreeMovie SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = FreeMovieOmni::makeStructRunner(
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(FreeMovieOmni::structfix($now, Struct::undef()))
                        !== json_encode(FreeMovieOmni::structfix($val, Struct::undef()))) {
