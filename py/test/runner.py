...
class FreeMovieTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            FreeMovieTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = FreeMovieTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = FreeMovieTestRunner.getenv("FREE_MOVIE_TEST_LIVE")
        override = FreeMovieTestRunner.getenv("FREE_MOVIE_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = FreeMovieTestRunner.getenv(key)
        ...
        explain = FreeMovieTestRunner.getenv("FREE_MOVIE_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["FREE_MOVIE_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if FreeMovieTestRunner._test_control is not None:
            return FreeMovieTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                FreeMovieTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            FreeMovieTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = FreeMovieTestRunner.load_test_control()
...
def load_env_local():
    FreeMovieTestRunner.load_env_local()


def env_override(m):
    return FreeMovieTestRunner.env_override(m)


def entity_data(v):
    return FreeMovieTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return FreeMovieTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return FreeMovieTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return FreeMovieTestRunner.load_test_control()


def live_client_options():
    return FreeMovieTestRunner.live_client_options()


def live_delay_ms():
    return FreeMovieTestRunner.live_delay_ms()
