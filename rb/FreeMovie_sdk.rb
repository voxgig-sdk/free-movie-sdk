...
require_relative 'FreeMovie_types'
...
class FreeMovieSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = FreeMovieHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = FreeMovieHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, FreeMovieFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    FreeMovieUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = FreeMovieHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = FreeMovieHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = FreeMovieHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => FreeMovieError.new(
        "#{op}_allow",
        "FreeMovieSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = FreeMovieHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = FreeMovieError.new(
        "graphql_error", "FreeMovieSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
