...
class MovieEntity
  ...
  def data_set(args)
    if args
      @_data = FreeMovieHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = FreeMovieHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [Movie, Hash] the loaded Movie; raises FreeMovieError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = FreeMovieHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
