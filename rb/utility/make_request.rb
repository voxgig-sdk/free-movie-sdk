...
module FreeMovieUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = FreeMovieResponse.new({})
    ...
    if fetch_err
      ...
      response = FreeMovieResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = FreeMovieResponse.new(fetched)
    else
