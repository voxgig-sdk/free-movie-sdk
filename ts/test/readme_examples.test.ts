...
//     (\`new FreeMovieSDK(...)\` / \`FreeMovieSDK.test(...)\` become
//     \`FreeMovieSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"movie":{"test01":{"id":"test01"}},"search":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new FreeMovieSDK(...)\` and any
// \`FreeMovieSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+FreeMovieSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/FreeMovieSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
