...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.FREEMOVIE_TEST_LIVE ||
    'TRUE' === process.env.FREEMOVIE_TEST_OVERRIDE
  ) {
  ...
  m.FREEMOVIE_TEST_EXPLAIN = process.env.FREEMOVIE_TEST_EXPLAIN || m.FREEMOVIE_TEST_EXPLAIN
